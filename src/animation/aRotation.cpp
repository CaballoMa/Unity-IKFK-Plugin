...
bool mat3::ToEulerAngles(RotOrder order, vec3& angleRad) const
{
    ...
    case YXZ:
        ...
        if (angleRad[VX] > -M_PI_2 + EPSILON)
        {
            if (angleRad[VX] < M_PI_2 - EPSILON)
            {
                angleRad[VY] = atan2(-mM[0][1], mM[1][1]);
                angleRad[VZ] = atan2(-mM[2][0], mM[2][2]);
                result = true;
            }
            else
            {
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    //q = (1 - u) * q0 + u * q1;
    float ab = q0.X() * q1.X() + q0.Y() * q1.Y() + q0.Z() * q1.Z();
    float angle = acos(ab);
    float coef_q0 = sin((1 - u) * angle);
    float coef_q1 = sin(u * angle) / sin(angle);
    q = coef_q0 * q0 + coef_q1 * q1;
    return q.Normalize();
}
...
quat quat::Scubic(const quat& b0, const quat& b1, const quat& b2, const quat& b3, double u)
{
    ...
    b01 = Slerp(b0, b1, u);
    b12 = Slerp(b1, b2, u);
    quat b23 = Slerp(b2, b3, u);
    quat b012 = Slerp(b01, b12, u);
    quat b122 = Slerp(b12, b23, u);
    result = Slerp(b012, b122, u);
...
void quat::ToAxisAngle(vec3& axis, double& angleRad) const
{
    ...
    axis[0] = mQ[VX] / sqrt(1 - mQ[VW] * mQ[VW]);
    axis[1] = mQ[VY] / sqrt(1 - mQ[VW] * mQ[VW]);
    axis[2] = mQ[VZ] / sqrt(1 - mQ[VW] * mQ[VW]);
}
...
mat3 quat::ToRotation() const
{
    ...
    m[0][0] = 1.0 - 2.0 * (mQ[VY] * mQ[VY] + mQ[VZ] * mQ[VZ]);
    m[0][1] = 2.0 * (mQ[VX] * mQ[VY] - mQ[VZ] * mQ[VW]);
    m[0][2] = 2.0 * (mQ[VX] * mQ[VZ] + mQ[VY] * mQ[VW]);

    m[1][0] = 2.0 * (mQ[VX] * mQ[VY] + mQ[VZ] * mQ[VW]);
    m[1][1] = 1.0 - 2.0 * (mQ[VX] * mQ[VX] + mQ[VZ] * mQ[VZ]);
    m[1][2] = 2.0 * (mQ[VY] * mQ[VZ] - mQ[VX] * mQ[VW]);

    m[2][0] = 2.0 * (mQ[VX] * mQ[VZ] - mQ[VY] * mQ[VW]);
    m[2][1] = 2.0 * (mQ[VY] * mQ[VZ] + mQ[VX] * mQ[VW]);
    m[2][2] = 1.0 - 2.0 * (mQ[VX] * mQ[VX] + mQ[VY] * mQ[VY]);
