...
void ASplineQuat::computeControlPoints(quat& startQuat, quat& endQuat)
{
	...
	for (int segment = 0; segment < numKeys - 1; segment++)
	{
		...
		{
			quat q2 = mKeys[segment + 2].second;
		}


		quat q0_sd, q1_sd, q1_sb, q_sb;
		q1_sd = quat::SDouble(q_1, q0);
		q1_sb = quat::SBisect(q1_sd, q1);

		q0_sd = quat::SDouble(q2, q1);
		q_sb = quat::SBisect(q0, q0_sd);


		b0 = q0;
		b1 = quat::Slerp(q0, q1_sb, 1 / 3);
		b2 = quat::Slerp(q1, q_sb, 1 / 3);
		b3 = q1;
